...
```http
GET /ovirt-engine/api/vms?search=name%3Dmyvm HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms?max=1 HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms?search=name%3D=myvm&case_sensitive=true HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms?search=page%201&max=10 HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms?search=page%202&max=10 HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms?search=sortby%20name%20page%202&max=10 HTTP/1.1
```
...
GET /ovirt-engine/api/vms/123/nics HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms/123?follow=disk_attachments,nics HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms/123?follow=disk_attachments.disk HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms/123?follow=disk_attachments.disk.statistics,nics.statistics HTTP/1.1
```
...
```http
GET /ovirt-engine/api/vms/123/permissions HTTP/1.1
```
